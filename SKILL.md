...
- **rembg Adapter** (`kb/tools/rembg_adapter.py`) — background removal via U2Net (gated, opt-in). CapCut "remove bg" equivalent.
- **auto-editor Adapter** (`kb/tools/auto_editor_adapter.py`) — auto silence/motion cut for long-form. EDL export to Premiere/Resolve/FCP.
- **MoviePy Adapter** (`kb/tools/moviepy_adapter.py`) — complex programmatic composition (HR#31 escape hatch for >5-node filtergraphs).
...
31. **FFmpeg filtergraphs exceeding 5 nodes must switch to MoviePy.** Complex composites (multi-clip PiP with animation, text with motion tracking, >5 overlapping elements) become unmaintainable and memory-unsafe in FFmpeg filtergraphs. Use `edit.moviepy_compose` instead.
