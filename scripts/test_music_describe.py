...
def _gen_multi_tone(path: str, sr: int = 22050) -> str:
    """Generate a 16-second track with 4 different tone segments."""
    segments = []
...
def test_describe_basics():
    ...
    print(f"  -> LUFS: {result['loudness_lufs']}, LRA: {result['dynamic_range_lra']}")
    print(f"  -> Vocals: {result['has_vocals']} (conf: {result['vocal_confidence']})")
    print(f"  -> Structure: {[(s['label'], round(s['start'],1), round(s['end'],1)) for s in result['structure']]}")
...
def test_scoring():
    ...
    print(f"  -> Score for 'sad piano': {s1:.2f}, for 'upbeat funk': {s2:.2f}")
...
def test_filter():
    ...
    check("filter: correct track", filtered2[0]["id"] == "c")
