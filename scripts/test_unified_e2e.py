...
print("=" * 60)
print("Unified Adapter E2E Smoke Test")
print("=" * 60)
...
# 3. Project file operations (no ffmpeg needed)
TMP = pathlib.Path(tempfile.mkdtemp(prefix="unified_e2e_"))
try:
    # Create a small test file for source
    source = TMP / "source.txt"
    source.write_text("test content")

    # project_create
    proj = edit.project_create(
        name="e2e_test",
        source=str(source),
        output_dir=str(TMP),
    )
    ...
    # project_step
    step = edit.project_step(proj["path"], {
        "operation": "probe", "tool": "ffprobe",
    ...
    step2 = edit.project_step(proj["path"], {
        "operation": "export", "tool": "ffmpeg",
        "input": str(source), "output": str(TMP / "out.mp4"),
        "params": {"crf": 22},
    })
...
# 4. ffmpeg-dependent ops (skip gracefully)
